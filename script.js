(() => {
  ...
  const API_BASE = "hhttps://mental-health-tracker-bbry.onrender.com/";
  ...
  // Stress chip wiring
  // ---------------------------------------------------------
  const chipGroup = document.getElementById("stress_level_group");
  ...
  function setMeter({ pct = 0, color = null, indeterminate = false, showDot = false } = {}) {
    const ARC = 283; // approx length of the semicircle path (pi * r=90)
    meterFill.classList.toggle("indeterminate", indeterminate);
    if (!indeterminate) {
      meterFill.style.strokeDashoffset = String(ARC * (1 - pct));
    }
    if (showDot) {
      ...
      meterDot.setAttribute("cx", cx.toFixed(1));
      meterDot.setAttribute("cy", cy.toFixed(1));
      meterDot.classList.add("show");
      if (color) meterDot.style.fill = color;
    } else {
      meterDot.classList.remove("show");
    }
  }
  ...
  function renderResult(score) {
    ...
    setMeter({ pct: clamped / 10, color, showDot: true });
  ...
  function renderError(label, copy) {
    ...
    scoreNumberEl.style.color = "var(--rose)";
    setMeter({ pct: 0 });
    showState("error");
  }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    setSubmitting(true);
    scoreNumberEl.textContent = "–";
    scoreNumberEl.style.color = "";
    setMeter({ indeterminate: true });
    showState("loading");
    ...
    try {
      ...
      if (res.status === 422) {
        ...
        renderError(
          "Check your inputs",
          matched
            ? "The API rejected a few fields — details are marked on the sheet."
  ...
  // initialize meter to idle state on load
  setMeter({ pct: 0 });
})();